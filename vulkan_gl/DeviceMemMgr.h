...
//------------------------------------------------------------------------------------------------------------------------------------------
// Manages vulkan device memory. Provides an interface to allocate both host visible/coherent memory (normally just RAM) as well as
// device local  memory (typically on-card GDDR5 etc.) via 'vkAllocateMemory'.
//
...
public:
    ...
    // Returns the size of the maxmium allocation that can be performed.
    // TODO: remove this limitation and add support for LARGE allocs that are not pooled.
    inline uint64_t getMaxAlloc() const noexcept { return MAIN_POOL_SIZE; }
...
private:
    ...
    static constexpr uint32_t MAIN_POOL_NUM_TIERS = 5;                          // Results in 256KB, 1MB, 4MB, 16MB, 64MB, 256MB tiers
    ...
    // Type for a memory pool id and constant representing an invalid pool id
    typedef uint32_t PoolIdT;
    static constexpr PoolIdT INVALID_POOL_ID = 0;

    //--------------------------------------------------------------------------------------------------------------------------------------
    // Type for a smaller pool which a parent pool to alloc/manage memory in smaller amounts
    //--------------------------------------------------------------------------------------------------------------------------------------
    struct SubPool {
        // Default initialize the subpool to an invalid pool
        inline SubPool() noexcept
            : offset(0)
            , subPoolId(INVALID_POOL_ID)
            , memMgr()
        {
        }

        // The offset of this subpool (in bytes) in the parent pool
        uint64_t offset;
    ...
    struct Pool {
        ...
        std::map<PoolIdT, SubPool> subPools;

        // The memory manager for this pool.
        // Provides 256MB of storage. Storage tiers: 256MB, 64MB, 16MB, 4MB, 1MB, 256KB
    ...
    PoolIdT pickNewPoolId() noexcept;
    PoolIdT pickNewSubPoolId(Pool& parentPool) noexcept;
    ...
    std::map<PoolIdT, Pool>     mPools;                 // Memory pools in the device memory manager
    std::vector<PoolIdT>        mDeviceLocalPools;      // A list of memory pools that are guaranteed local to the device. These probably won't be CPU/host visible unless there is a shared memory architecture.
    std::vector<PoolIdT>        mHostVisiblePools;      // A list of memory pools that are guaranteed host visible. These probably won't be on the device unless there is a shared memory architecture.
    PoolIdT                     mNextNewPoolId;         // Use this id for the next memory pool to create
