...
void WadList::finalize() noexcept {
    ...
    for (int32_t wadFileIndex = 0; wadFileIndex < (int32_t) mWadFiles.size(); ++wadFileIndex) {
        ...
        for (int32_t lumpIdx = 0; lumpIdx < numWadLumps; ++lumpIdx) {
            const WadFile::LumpName lumpName = wadFile.getLumpName(lumpIdx);
            mLumpHandles.push_back({ wadFileIndex, lumpIdx, lumpName.word & WadFile::LUMPNAME_MASK });      // Note: remove the special 'compressed' flag bit to make later search a bit faster
        }
    }
}
...
//------------------------------------------------------------------------------------------------------------------------------------------
int32_t WadList::findLumpIdx(const char* const lumpName, const int32_t searchStartIdx) const noexcept {
    const uint64_t lumpNameWord = WadFile::makeLumpNameWord(lumpName);
    ...
    for (int32_t i = searchStartIdx; i < numLumps; ++i) {
        if (pLumpHandles[i].nameWord == lumpNameWord)
            return i;
    }
...
//------------------------------------------------------------------------------------------------------------------------------------------
const WadFile::Lump& WadList::getLump(const int32_t lumpIdx) const noexcept {
    ASSERT((lumpIdx >= 0) && (lumpIdx < getNumLumps()));
...
//------------------------------------------------------------------------------------------------------------------------------------------
const WadFile::LumpName WadList::getLumpName(const int32_t lumpIdx) const noexcept {
    ASSERT((lumpIdx >= 0) && (lumpIdx < getNumLumps()));
    const LumpHandle& lumpHandle = mLumpHandles[lumpIdx];
    const WadFile& wadFile = mWadFiles[lumpHandle.wadFileIdx];
...
//------------------------------------------------------------------------------------------------------------------------------------------
const WadFile::Lump& WadList::cacheLump(const int32_t lumpIdx, const int16_t allocTag, const bool bDecompress) noexcept {
    ASSERT((lumpIdx >= 0) && (lumpIdx < getNumLumps()));
    const LumpHandle& lumpHandle = mLumpHandles[lumpIdx];
    WadFile& wadFile = mWadFiles[lumpHandle.wadFileIdx];
