...
uint32_t gWess_Millicount;
uint32_t gWess_Millicount_Frac;
...
static uint32_t     gWess_T2counter;            // Tracks the number of PSX timer 2 interrupts or calls to 'WessInterruptHandler'
static uint32_t     gWess_EV2;                  // Hardware event handle for the PSX timer 2 interrupt event which drives the music system
static PsxCd_File   gWess_module_fileref;       // Holds the current file open by the module loader
static PsxCd_File   gWess_data_fileref;         // Holds the current file open by the data loader
static bool         gbWess_ReadChunk1;          // If true we can read data to sector buffer 1
static bool         gbWess_ReadChunk2;          // If true we can read data to sector buffer 2
...
static void data_read_chunk(PsxCd_File& file, const int32_t chunkSize, const uint32_t spuDestAddr) noexcept {
    ...
    if (gbWess_ReadChunk1) {
        ...
        psxcd_read(gWess_sectorBuffer1, chunkSize, file);
        LIBSPU_SpuSetTransferStartAddr(spuDestAddr);
        LIBSPU_SpuWrite(gWess_sectorBuffer1, chunkSize);
        gbWess_ReadChunk2 = true;
        gbWess_ReadChunk1 = false;
    } else {
        if (gbWess_ReadChunk2) {
            // Reading to chunk 2
            psxcd_read(gWess_sectorBuffer2, chunkSize, file);
            LIBSPU_SpuIsTransferCompleted(SPU_TRANSFER_WAIT);
            LIBSPU_SpuSetTransferStartAddr(spuDestAddr);
            LIBSPU_SpuWrite(gWess_sectorBuffer2, chunkSize);
