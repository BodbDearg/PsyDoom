...
struct GameSettings {
    ...
    uint8_t     bNoFriendlyFire;                // If true then players are not able to damage each other in coop.
    uint8_t     bMPThings;                      // Enable multiplayer-only things in co-op.
    uint8_t     bExitDisabled;                  // If true while playing deathmatch, exit will display message and do nothing. 
    uint8_t     bPreserveKeys;                  // If true while playing co-op a player will spawn with previously collected keys.
