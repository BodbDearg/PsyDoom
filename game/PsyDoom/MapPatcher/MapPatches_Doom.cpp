...
static void patchMap_PhobosAnomaly() noexcept {
    ...
    if (shouldApplyMapPatches_GamePlay() && (gNetGame == gt_deathmatch || Game::gSettings.bNoMonsters)) {
        // The wall on the north tip of the star should already be lowered for deathmatch or if 'no monsters' is set
        gpSectors[65].floorheight = -136 * FRACUNIT;
    }
}
...
static void patchMap_TowerOfBabel() noexcept {
    ...
    if (shouldApplyMapPatches_GamePlay() && (gNetGame == gt_deathmatch || Game::gSettings.bNoMonsters)) {
        // The exit door should already be opened for deathmatch or if 'no monsters' is set
        gpSectors[30].ceilingheight = 124 * FRACUNIT;
    }
}
...
static void patchMap_UntoTheCruel() noexcept {
    ...
    if (shouldApplyMapPatches_GamePlay() && (gNetGame == gt_deathmatch || Game::gSettings.bNoMonsters)) {
        // The wall around the exit should already be lowered for deathmatch or if 'no monsters' is set
...
//------------------------------------------------------------------------------------------------------------------------------------------
// Fix issues for MAP57: The Marshes
//------------------------------------------------------------------------------------------------------------------------------------------
static void patchMap_TheMarshes() noexcept {
    applyOriginalMapCommonPatches();

    if (shouldApplyMapPatches_GamePlay() && (gNetGame == gt_deathmatch || Game::gSettings.bNoMonsters)) {
        // The exit door should already be opened for deathmatch or if 'no monsters' is set
        gpSectors[34].ceilingheight = 68 * FRACUNIT;
    }
}
...
static const PatchDef gPatchArray_Doom[] = {
    ...
    {  58333, 0xBBE3159AAEE4F03D, 0x086F778E5A08DBAD, patchMap_TheMarshes           },      // MAP57
